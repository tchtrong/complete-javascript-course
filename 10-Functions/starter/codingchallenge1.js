...
const poll = {
  ...
  displayResults(type = 'array') {
    if (typeof type !== 'string') throw new TypeError('Wrong type of param');
  ...
  registerNewAnswer() {
    ...
    for (const [idx, option] of this.options.entries()) {
      selectionStr = selectionStr.concat(`${idx}: ${option}\n`);
    }
    const promptStr =
      this.question + '\n' + selectionStr + '(Write option number)';
    const ans = Number(prompt(promptStr));
    if (isNaN(ans) || ans < 0 || ans > this.options.length - 1) {
      alert('Wrong input');
    } else {
      this.answers[ans] += 1;
    }
